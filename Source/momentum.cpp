...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(velCont[0]); mfi.isValid(); ++mfi)
    {
        ...
        auto const &vel_star_diff_x = velStarDiff[0].array(mfi);
        auto const &vel_star_diff_y = velStarDiff[1].array(mfi);
#if (AMREX_SPACEDIM > 2)
        auto const &vel_star_diff_z = velStarDiff[2].array(mfi);
#endif
        ...
        amrex::ParallelFor(xbx, 
            ...
            xrhs(i, j, k) = xrhs(i, j, k) - ( Real(1.5)/dt )*vel_star_diff_x(i, j, k) + ( Real(0.5)/dt )*vel_cont_diff_x(i, j, k);

            vel_star_x(i, j, k) = vel_cont_x(i, j, k) + ( rk[sub] * xrhs(i, j, k) );

            vel_star_diff_x(i, j, k) = vel_star_x(i, j, k) - vel_cont_x(i, j, k);
        });
        
        amrex::ParallelFor(ybx,
                           [=] AMREX_GPU_DEVICE(int i, int j, int k) {
            yrhs(i, j, k) = yrhs(i, j, k) - ( Real(1.5)/dt )*vel_star_diff_y(i, j, k) + ( Real(0.5)/dt )*vel_cont_diff_y(i, j, k);
                        
            vel_star_y(i, j, k) = vel_cont_y(i, j, k) + ( rk[sub] * yrhs(i, j, k) );

            vel_star_diff_y(i, j, k) = vel_star_y(i, j, k) - vel_cont_y(i, j, k);
        });
#if (AMREX_SPACEDIM > 2)
        amrex::ParallelFor(zbx,
                           [=] AMREX_GPU_DEVICE(int i, int j, int k) {
            zrhs(i, j, k) = zrhs(i, j, k) - ( Real(1.5)/dt )*vel_star_diff_z(i, j, k) + ( Real(0.5)/dt )*vel_cont_diff_z(i, j, k);
                        
            vel_star_z(i, j, k) = vel_cont_z(i, j, k) + ( rk[sub] * zrhs(i, j, k) );

            vel_star_diff_z(i, j, k) = vel_star_z(i, j, k) - vel_cont_z(i, j, k);
        });
#endif
    }
