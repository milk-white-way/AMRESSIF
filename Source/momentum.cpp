...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(velHat[0]); mfi.isValid(); ++mfi)
    {
        ...
        amrex::ParallelFor(xbx, 
            ...
            vel_hat_x(i, j, k) = vel_hat_x(i, j, k) + ( rk[sub] * dt * Real(0.6) * xrhs(i, j, k) );
        ...
        amrex::ParallelFor(ybx,
            ...
            vel_hat_y(i, j, k) = vel_hat_y(i, j, k) + ( rk[sub] * dt * Real(0.6) * yrhs(i, j, k) );
        ...
        amrex::ParallelFor(zbx,
            ...
            vel_hat_z(i, j, k) = vel_hat_z(i, j, k) + ( rk[sub] * dt * Real(0.6) * zrhs(i, j, k) );
