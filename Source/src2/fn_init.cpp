...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for ( MFIter mfi(velCont[0]); mfi.isValid(); ++mfi )
    {
        ...
        amrex::ParallelFor(xbx,
            ...
            vel_cont_prev_x(i, j, k) = std::sin(amrex::Real(2.0) * M_PI * x) * std::cos(amrex::Real(2.0) * M_PI * y) * std::exp(- amrex::Real(8.0) * M_PI * M_PI * (time - dt));
        ...
        amrex::ParallelFor(ybx,
            ...
            vel_cont_prev_y(i, j, k) = - std::cos(amrex::Real(2.0) * M_PI * x) * std::sin(amrex::Real(2.0) * M_PI * y) * std::exp(- amrex::Real(8.0) * M_PI * M_PI * (time - dt));
        ...
        amrex::ParallelFor(zbx,
                           [=] AMREX_GPU_DEVICE(int i, int j, int k){
            vel_cont_z(i, j, k) = amrex::Real(0.1);
     ...
     * 
     */
    cont2cart(velCart, velCont, geom, Nghost, phy_bc_lo, phy_bc_hi, time, n_cell);
    amrex::Print() << "\n";
    cont2cart(velCartPrev, velContPrev, geom, Nghost, phy_bc_lo, phy_bc_hi, time, n_cell);
    amrex::Print() << "\n";
