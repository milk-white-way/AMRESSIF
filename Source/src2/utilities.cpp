...
#pragma omp parallel if (Gpu::notInLaunchRegion())
   ...
   for (MFIter mfi(velCart); mfi.isValid(); ++mfi) {
      ...
      int hi = dom.bigEnd(0);
		...
		// IMPORTANT: CURRENTLY SET TO TAYLOR GREEN VORTEX
         ...
         if (west_wall_bcs == 135) {
            amrex::ParallelFor(vbx,
				...
				amrex::ParallelFor(vbx,
					...
					if (i < lo) {
						for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
							vel_cart(i, j, k, dir) = 2 - vel_cart(-i - 1, j, k, dir);
						}
					}
				});
			} else if (west_wall_bcs == -165) {
				amrex::ParallelFor(vbx,
            		...
            		amrex::Real y = prob_lo[1] + (j + Real(0.5)) * dx[1];
						...
						vel_cart(i, j, k, 0) = 2*( std::sin(amrex::Real(2.0) * M_PI *x) * std::cos(amrex::Real(2.0) * M_PI * y) * std::exp(- amrex::Real(8.0) * M_PI * M_PI * time) ) - vel_cart(-i - 1, j, k, 0);
						vel_cart(i, j, k, 1) = 2* ( -std::cos(amrex::Real(2.0) * M_PI *x) * std::sin(amrex::Real(2.0) * M_PI * y) * std::exp(- amrex::Real(8.0) * M_PI * M_PI * time )) - vel_cart(-i - 1, j, k, 1);
					}
				});
		...
		if (vbx.bigEnd(0) > hi) {
			if (east_wall_bcs == 135) {
				...
				amrex::ParallelFor(vbx,
					...
					if (i > hi) {
						for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
							vel_cart(i, j, k, dir) = 2 - vel_cart(((n_cell - i) + (n_cell - 1)), j, k, dir);
						}
					}
				});
			} else if (east_wall_bcs == -165) {
				amrex::ParallelFor(vbx,
            		...
            		amrex::Real y = prob_lo[1] + (j + Real(0.5)) * dx[1];
						...
						vel_cart(i, j, k, 0) = 2*( std::sin(amrex::Real(2.0) * M_PI *x) * std::cos(amrex::Real(2.0) * M_PI * y) * std::exp(- amrex::Real(8.0) * M_PI * M_PI * time) ) - vel_cart(((n_cell - i) + (n_cell - 1)), j, k, 0);
						vel_cart(i, j, k, 1) = 2* ( -std::cos(amrex::Real(2.0) * M_PI *x) * std::sin(amrex::Real(2.0) * M_PI * y) * std::exp(- amrex::Real(8.0) * M_PI * M_PI * time )) - vel_cart(((n_cell - i) + (n_cell - 1)), j, k, 1);
					}
				});
		...
		if (vbx.smallEnd(1) < lo) {
			if (south_wall_bcs == 135) {
				...
				amrex::ParallelFor(vbx,
					...
					if (j < lo) {
						for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
							vel_cart(i, j, k, dir) = 2 - vel_cart(i, -j - 1, k, dir);
						}
					}
				});
			} else if (south_wall_bcs == -165) {
				amrex::ParallelFor(vbx,
            		...
            		amrex::Real y = prob_lo[1] + (j + Real(0.5)) * dx[1];
						...
						vel_cart(i, j, k, 0) = 2*( std::sin(amrex::Real(2.0) * M_PI *x) * std::cos(amrex::Real(2.0) * M_PI * y) * std::exp(- amrex::Real(8.0) * M_PI * M_PI * time) ) - vel_cart(i, -j - 1, k, 0);
						vel_cart(i, j, k, 1) = 2* ( -std::cos(amrex::Real(2.0) * M_PI *x) * std::sin(amrex::Real(2.0) * M_PI * y) * std::exp(- amrex::Real(8.0) * M_PI * M_PI * time )) - vel_cart(i, -j - 1, k, 1);
					}
				});
			...
			if (north_wall_bcs == 135) {
				...
				amrex::ParallelFor(vbx,
            		...
            		amrex::Real y = prob_lo[1] + (j + Real(0.5)) * dx[1];
						...
						vel_cart(i, j, k, 0) = 2*( std::sin(amrex::Real(2.0) * M_PI *x) * std::cos(amrex::Real(2.0) * M_PI * y) * std::exp(- amrex::Real(8.0) * M_PI * M_PI * time) ) - vel_cart(i, ((n_cell - j) + (n_cell - 1)), k, 0);
						vel_cart(i, j, k, 1) = 2* ( -std::cos(amrex::Real(2.0) * M_PI *x) * std::sin(amrex::Real(2.0) * M_PI * y) * std::exp(- amrex::Real(8.0) * M_PI * M_PI * time )) - vel_cart(i, ((n_cell - j) + (n_cell - 1)), k, 1);
					}
				});
