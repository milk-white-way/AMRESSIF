...
void main_main ()
{
	...
	int n_cell, max_grid_size, nsteps, plot_int, checkpoint;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-= Defining System's Variables =-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	*/
	...
	if (checkpoint == 0) {
		...
		amrex::Print() << "INFO | Request from user to load checkpoints" << "\n";
	}
	...
	for (int n = 1; n <= nsteps; ++n)
	{
		...
		while ( normError > momentum_tolerance )
		{
			...
			if ( PSEUDO_TIMESTEPPING == 0 ) {
				...
				// --------------------------- MOMENTUM SOLVER ---------------------------
				explicit_time_marching(momentum_rhs, velStar, velCont, velContDiff, velContPrev, velCart, geom, Nghost, phy_bc_lo, phy_bc_hi, n_cell, dt, time);
			} else {
				...
				for (int sub = 0; sub < RungeKuttaOrder; ++sub )
				{
					...
					// --------------------------- MOMENTUM SOLVER ---------------------------
					runge_kutta4_pseudo_time_stepping(rk, sub, momentum_rhs, velStar, velCont, velContDiff, velContPrev, velCart, geom, Nghost, phy_bc_lo, phy_bc_hi, n_cell, dt, time);
		...
		update_solution(array_grad_p, array_grad_phi, fluxPrsGrad, cc_grad_phi, poisson_rhs, userCtx, velCart, velCont, velContPrev, velContDiff, geom, dt, time, Nghost, phy_bc_lo, phy_bc_hi, n_cell, ren);
		gradient_calc_approach2(array_grad_p, array_grad_phi, userCtx, geom, Nghost, phy_bc_lo, phy_bc_hi, n_cell);
