...
void main_main ()
{
    ...
    //    the components supposedly are:
    //      vel_x::velocity in the x-axis
    //      vel_y::velocity in the y-axis
    //      press::pressure
    //      NOTE: all are cell-centered
    int Ncomp  = 3;

    // In this version, let's consider them to be three seperated heat sources
    ...
    // Let us create the first MultiFab
    MultiFab userctx(ba, dm, Ncomp, Nghost);

    GpuArray<Real,AMREX_SPACEDIM> dx = geom.CellSizeArray();

    init_phi(userctx, geom);
    // ========================================
    ...
    if (plot_int > 0)
    {
        ...
        WriteSingleLevelPlotfile(pltfile, userctx,
                                 {"heat_1",
                                  "heat_2",
                                  "heat_3"},
                                 geom, time, 0);
...
/*
    ...
    MultiFab user_context_old(ba, dm, Ncomp, Nghost);
