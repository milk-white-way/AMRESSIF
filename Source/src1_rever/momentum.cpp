    ...
    for (MFIter mfi(velCont[0]); mfi.isValid(); ++mfi)
    {
        ...
        amrex::ParallelFor(xbx, 
            ...
            vel_star_x(i, j, k) = vel_cont_x(i, j, k) + ( rk[sub] * xrhs(i, j, k) );
				//amrex::Print() << "DEBUG | velStar[0] during 1st iter of RK4 at i=" << i << " ; j=" << j << " ; k=" << k << " = " << vel_star_x(i, j, k) << "\n";
        });
        ...
        amrex::ParallelFor(ybx,
            ...
            vel_star_y(i, j, k) = vel_cont_y(i, j, k) + ( rk[sub] * yrhs(i, j, k) );
				//amrex::Print() << "DEBUG | velStar[1] during 1st iter of RK4 at i=" << i << " ; j=" << j << " ; k=" << k << " = " << vel_star_y(i, j, k) << "\n";
        });
#if (AMREX_SPACEDIM > 2)
        amrex::ParallelFor(zbx,
            ...
            vel_star_z(i, j, k) = vel_cont_z(i, j, k) + ( rk[sub] * zrhs(i, j, k) );
				amrex::Print() << "DEBUG | velStar[2] during 1st iter of RK4 at i=" << i << " ; j=" << j << " ; k=" << k << " = " << vel_star_z(i, j, k) << "\n";
        });
#endif
    }
