...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for ( MFIter mfi(rhs[0]); mfi.isValid(); ++mfi )
    {
        ...
        amrex::ParallelFor(xbx,
				...
				//amrex::Print() << "DEBUG | Face cell at i=" << i << " ; j=" << j << " has pressure gradient = " << grad_p_x(i, j, k) << "\n";
            xrhs(i, j, k) = - grad_p_x(i, j, k) + amrex::Real(0.5)*( total_flux(i-1, j, k, 0) + total_flux(i, j, k, 0) );
        });

        amrex::ParallelFor(ybx,
                           [=] AMREX_GPU_DEVICE (int i, int j, int k){ 
				//amrex::Print() << "DEBUG | Face cell at i=" << i << " ; j=" << j << " has pressure gradient = " << grad_p_y(i, j, k) << "\n";
