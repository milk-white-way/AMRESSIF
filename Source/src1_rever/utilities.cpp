            ...
            amrex::ParallelFor(vbx,
					...
					if (i < lo) {
						for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
							...
							//amrex::Print() << "DEBUG | Filling ghost cell at i=" << i << " ; j=" << j << " = " << vel_cart(i, j, k, dir) << "\n";
							//amrex::Print() << "DEBUG | Filling ghost cell at i=" << i << " ; j=" << j << " ; k=" << k << " = " << vel_cart(i, j, k, dir) << "\n";
						}
					}
				});
			...
			if (north_wall_bcs == 135) {
				...
				amrex::ParallelFor(vbx,
					...
					if (j > hi) {
							...
							vel_cart(i, j, k, 2) = -vel_cart(i, ((n_cell - j) + (n_cell - 1)), k, 2);
#endif
							//amrex::Print() << "DEBUG | Filling ghost cell at i=" << i << " ; j=" << j << " u = " << vel_cart(i, j, k, 0) << "\n";
							//amrex::Print() << "DEBUG | Filling ghost cell at i=" << i << " ; j=" << j << " v = " << vel_cart(i, j, k, 1) << "\n";
					}
				});
			} else if (north_wall_bcs == -165) {
				amrex::Abort("Not yet unlocked");
